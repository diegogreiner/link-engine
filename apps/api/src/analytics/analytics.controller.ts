...
export class AnalyticsController {
	...
	getClicksPerDay(
		...
		return this.analyticsService.getClicksPerDay(
			linkId,
			req.user["sub"],
			filter,
		);
	...
	@Get(":id/by-country")
	getByCountry(@Param("id") linkId: string, @Req() req: Request) {
		return this.analyticsService.getByCountry(linkId, req.user["sub"]);
	}

	@Get(":id/by-device")
	getByDevice(@Param("id") linkId: string, @Req() req: Request) {
		return this.analyticsService.getByDevice(linkId, req.user["sub"]);
	}

	@Get(":id/by-referrer")
	getByReferrer(@Param("id") linkId: string, @Req() req: Request) {
